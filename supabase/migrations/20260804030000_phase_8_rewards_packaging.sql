...
create index if not exists reward_submissions_status_created_idx
  on public.reward_submissions (status, created_at desc);

drop trigger if exists reward_submissions_set_updated_at; create trigger reward_submissions_set_updated_at
...
create index if not exists reward_vouchers_email_idx on public.reward_vouchers (customer_email);
create index if not exists reward_vouchers_active_idx on public.reward_vouchers (code)
  where is_used = false;

drop trigger if exists reward_vouchers_set_updated_at; create trigger reward_vouchers_set_updated_at
...
drop policy if exists admin_all_reward_submissions; create policy admin_all_reward_submissions
  on public.reward_submissions for all
  to authenticated
  using ((select public.is_admin()))
  with check ((select public.is_admin()));

drop policy if exists customer_read_own_reward_submissions; create policy customer_read_own_reward_submissions
...
drop policy if exists admin_read_reward_vouchers; create policy admin_read_reward_vouchers
  on public.reward_vouchers for select
  to authenticated
  using ((select public.is_admin()));

drop policy if exists customer_read_own_reward_vouchers; create policy customer_read_own_reward_vouchers
  on public.reward_vouchers for select
  to authenticated
  using (customer_email = (select (auth.jwt() ->> 'email')));
