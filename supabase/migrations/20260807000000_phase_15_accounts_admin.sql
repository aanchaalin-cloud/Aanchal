...
drop trigger if exists customer_addresses_set_updated_at; create trigger customer_addresses_set_updated_at
...
drop policy if exists customer_select_own_addresses; create policy customer_select_own_addresses
  on public.customer_addresses for select
  to authenticated
  using (customer_id = (select auth.uid()));

drop policy if exists customer_insert_own_addresses; create policy customer_insert_own_addresses
  on public.customer_addresses for insert
  to authenticated
  with check (customer_id = (select auth.uid()));

drop policy if exists customer_update_own_addresses; create policy customer_update_own_addresses
  on public.customer_addresses for update
  to authenticated
  using (customer_id = (select auth.uid()))
  with check (customer_id = (select auth.uid()));

drop policy if exists customer_delete_own_addresses; create policy customer_delete_own_addresses
  on public.customer_addresses for delete
  to authenticated
  using (customer_id = (select auth.uid()));

drop policy if exists admin_read_customer_addresses; create policy admin_read_customer_addresses
  on public.customer_addresses for select
  to authenticated
