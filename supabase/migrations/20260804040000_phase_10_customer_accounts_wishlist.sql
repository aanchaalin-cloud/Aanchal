...
create policy customer_select_own_wishlist
  on public.wishlist_items for select
  to authenticated
  using (customer_id = (select auth.uid()));

create policy customer_insert_own_wishlist
  on public.wishlist_items for insert
  to authenticated
  with check (customer_id = (select auth.uid()));

create policy customer_update_own_wishlist
  on public.wishlist_items for update
  to authenticated
  using (customer_id = (select auth.uid()))
  with check (customer_id = (select auth.uid()));

create policy customer_delete_own_wishlist
